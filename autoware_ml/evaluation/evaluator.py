...
backend), preprocesses through the model's ``on_after_batch_transfer`` (the one place
preprocessing runs), reassembles the final graph outputs into what ``forward()`` returns,
decodes through the model's ``build_eval_output`` and scores with fresh clones of its own
...
class EvaluationResult:
    ...
    Attributes:
        ...
        split: Evaluated split (``test`` / ``val``).
...
def _batch_size(model: Any, batch: Any) -> int:
    size = model.get_log_batch_size(batch) if hasattr(model, "get_log_batch_size") else None
...
def evaluate_backend(
    ...
    *,
    ...
    Args:
        model: Model exposing ``on_after_batch_transfer``, ``build_eval_output``,
            ``clone_metrics`` and ``get_log_batch_size`` (:class:`BaseModel`).
        dataloader: Batches of the split to evaluate, with ground truth.
    ...
    for index, batch in enumerate(dataloader):
        ...
        batch = model.on_after_batch_transfer(move_to_device(batch, device), dataloader_idx=0)
        _sync(device)
        ...
        eval_out = model.build_eval_output(batch, outputs)
        if index == 0:
        ...
        evaluated += _batch_size(model, batch)
...
def log_backend_report(result: EvaluationResult) -> None:
    ...
    for key, value in sorted(
        (k, v) for k, v in result.metrics.items() if is_headline(k, result.headline_metrics)
    ):
...
def log_comparison(results: Sequence[EvaluationResult]) -> None:
    ...
    for result in results:
        keys = {
            ...
            if is_headline(k, result.headline_metrics)
        }
    ...
    logger.info("    %-40s%s", "metric", "".join(f"{label(r):>12}" for r in results))
    for row in sorted(rows or ()):
        ...
        logger.info("    %-40s%s", row, values)
    latency_row = "".join(
        f"{(r.latency[MODEL_STAGE].mean if MODEL_STAGE in r.latency else 0.0):>12.3f}"
        for r in results
    )
    logger.info("    %-40s%s", MODEL_STAGE + " mean [ms]", latency_row)
    for result in results:
        if result.fallback_stages:
            logger.info(
...
def flatten_results(results: Sequence[EvaluationResult]) -> dict[str, float]:
    """Every result's metrics (canonical keys) plus mean latencies, as one flat mapping."""
    flat: dict[str, float] = {}
    for result in results:
        flat.update(result.metrics)
        for name, stats in result.latency.items():
            flat[latency_key(result.backend, f"{name}_mean_ms")] = stats.mean
    return flat
