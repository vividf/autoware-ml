...
def _resolve_ann_file(data_root: str, ann_file: str) -> str:
    """Resolve an annotation file path against the dataset root.

    Args:
        data_root: Dataset root directory.
        ann_file: Absolute path, or a path relative to ``data_root``.

    Returns:
        Absolute annotation path, or the ``data_root``-joined relative path.
    """
...
def build_streaming_dataloader(
    ...
    *,
    ...
    Raises:
        ValueError: If the dataloader configuration requests shuffling.
    """
    if dataloader_cfg.shuffle:
        raise ValueError(
    ...
    return DataLoader(
        ...
        persistent_workers=dataloader_cfg.persistent_workers and dataloader_cfg.num_workers > 0,
...
class MultiviewDetection3DDataModule(DataModule):
    ...
    def __init__(
        ...
        Args:
            ...
            streaming: Whether every dataloader streams scene-contiguous frames
                per batch lane. Stateful temporal detectors such as StreamPETR
        ...
        super().__init__(
            ...
            predict_dataloader_cfg=predict_dataloader_cfg,
        )
        self.data_root = data_root
        self.class_names = list(class_names)
        self.camera_order = list(camera_order)
        self.name_mapping: Mapping[str, str] = MappingProxyType(dict(name_mapping or {}))
        ...
        self.ann_files = {
            ...
            "test": _resolve_ann_file(data_root, test_ann_file),
            "predict": _resolve_ann_file(data_root, test_ann_file),
        }
