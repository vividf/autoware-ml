...
class VoVNetMultiScale(nn.Module):
    ...
    def __init__(
        ...
        supported_features = ("stem", "stage2", "stage3", "stage4", "stage5")
        unknown = [name for name in self.out_features if name not in supported_features]
        if unknown:
            raise ValueError(
                f"Unsupported out_features {unknown}; expected names from {supported_features}."
            )
