...
class StreamPETRHead(nn.Module):
    ...
    def _loss_single(
        ...
        if any(sample_targets.label_weights is not None for sample_targets in targets):
            label_weights_tensor = torch.stack(
                [
                    sample_targets.label_weights
                    if sample_targets.label_weights is not None
                    else cls_scores.new_ones((sample_targets.labels.shape[0], self.num_classes))
                    for sample_targets in targets
                ],
                dim=0,
            )
        # Global (cross-rank) positive count, following DETR's num_boxes
        # all-reduce and mmdetection's reduce_mean: with DDP averaging the
        # gradients, dividing by the mean count on every rank yields the true
        # per-object mean over the whole effective batch. The rank-local count
        # used before was biased upward by ~8% here (per-frame counts vary a
        # lot) and made the result depend on the GPU count. cls and bbox share
        # the same factor, so their relative weighting is unchanged; the
        # overall 3D-head scale shifts by the removed bias, which perturbs the
        # recipe tuned before this change — see
        # docs/framework/streampetr-alignment-report.md section 6.4.
        # One collective + device sync per decoder layer (uniform across
        # ranks: this method runs unconditionally for every layer).
    ...
    def dn_loss_single(
        ...
        # a ball of radius ``split`` inside the unit noise cube. cls and bbox
        # scale with the same (global) count, so their relative weighting is
        # invariant to the normalization change.
