...
def update_ego_poses(
    ...
    Streaming temporal models warp the previous frame's memory with
    ``ego_pose_inv(t) @ ego_pose(t-1)``, so after augmenting frame ``t`` the
    pose pair must map between the *augmented* lidar frame and the global
    ...
    input_dict["ego_pose"] = ego_pose @ aug_inv.astype(ego_pose.dtype)
    input_dict["ego_pose_inv"] = aug.astype(ego_pose_inv.dtype) @ ego_pose_inv
