...
class LoadAnnotations2DFromBoxes3D(BaseTransform):
    ...
    Outputs (per camera, one entry per visible box):
        ...
        ``depths_2d``: ``(N,)`` metric distance of each box center.
    ...
    def transform(self, input_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        all_bboxes, all_centers, all_depths, all_labels = [], [], [], []
        ...
        for camera_index in range(num_cams):
            ...
            bboxes, centers, depths, labels = self._project_camera(
                corners,
                gravity_centers,
                gt_boxes,
                gt_labels,
    ...
    def _project_camera(
        ...
        valid_bboxes, valid_centers, valid_depths, valid_labels = [], [], [], []
        for box_index in range(gt_boxes.shape[0]):
            ...
            valid_depths.append(float(np.linalg.norm(gt_boxes[box_index, :3])))
        ...
        if valid_bboxes:
            return (
                ...
                np.asarray(valid_depths, dtype=np.float32),
