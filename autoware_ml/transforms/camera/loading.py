...
class LoadMultiViewImagesFromFiles(BaseTransform):
        ...
        *,
        ...
        Args:
            ...
            shuffle_order: Shuffle the camera order per sample (train-time
                regularization for camera-order-agnostic models).
        """
        self.to_float32 = to_float32
    ...
    def transform(self, input_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        lidar2img = []
        camera_names = []
        camera_order = input_dict["camera_order"]
        if self.shuffle_order:
            camera_order = [camera_order[i] for i in np.random.permutation(len(camera_order))]
