...
def load_quantized_model(
    ...
    *,
    ...
    if uncovered and not enforce_full_coverage:
        logger.warning(
            "%d model parameter(s) not covered by the checkpoint(s) (first: %s).",
            len(uncovered),
            ", ".join(uncovered[:5]),
        )
    elif uncovered:
...
def load_model_weights(
    ...
    *,
    ...
    """Load ``--weights`` into a freshly built model, quantized or not.

    The one entry point the deploy / test scripts call: when exactly one of the
    checkpoints carries an embedded quantization description, the identical quantized
    module tree is rebuilt from it before the weights load
    (:func:`load_quantized_model`); otherwise the plain
    :func:`~autoware_ml.utils.checkpoints.apply_matching_weights` runs as before. Callers
    never read a ``quantization`` config section — an INT8 checkpoint deploys and scores
    ...
    weight_paths = [Path(path) for path in weight_paths]
    quantized = find_quantization(weight_paths)
    if quantized is not None:
        path, description = quantized
        logger.info(
            "Quantized checkpoint detected (%s, mode=%s): rebuilding the quantized tree from "
            "its embedded description.",
            path,
            description.config.mode,
        )
        load_quantized_model(
            model, weight_paths, description, device, enforce_full_coverage=enforce_full_coverage
        )
        if not set_eval:
            model.train()
        return
