...
class _ParityReport:
    ...
    def check_tensor(
        ...
        if actual.shape != expected.shape:
            self.rows.append((f"{name} shape {tuple(actual.shape)} vs {tuple(expected.shape)}", float("nan"), float("nan"), False))
            return
...
def main() -> None:
    ...
    with torch.no_grad():
        ...
        report.check_tensor(
            "roi_cls (isolated)", roi_isolated["enc_cls_scores"], reference["outputs"]["roi_enc_cls_scores"]
        )
        report.check_tensor(
            "roi_bbox (isolated)", roi_isolated["enc_bbox_preds"], reference["outputs"]["roi_enc_bbox_preds"]
        )
        report.check_tensor(
