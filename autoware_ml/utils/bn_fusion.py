...
"""Fold BatchNorm into the preceding convolution / linear layer.

Deployed graphs carry no BatchNormalization node: the fold is an inference identity,
removes an op per layer, and is what quantization later calibrates on (the weight
quantizer must see the folded weight the deployed graph actually carries). Pairs are
taken only from structure that *proves* dataflow — a container that applies its children
in order, or a block that declares ``bn_fusion_pairs`` — never guessed from registration
order.
"""
...
def find_conv_bn_pairs(model: nn.Module) -> list[tuple[str, str]]:
    ...
    for parent_name, parent in model.named_modules():
        ...
        for (left_name, left), (right_name, right) in pairwise(children):
...
def bn_folded_copy(module: nn.Module) -> nn.Module:
    """Return ``module`` with BatchNorm folded on a deep copy, or ``module`` itself if none.
